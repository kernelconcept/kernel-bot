...
from kernel import text, enrich_user_id, enrich_role_name, has_admin
...
class Profile:
    ...
    async def init(self):
        ...
        if count:
            if count == 1:
                await self.bot.reply(text.INIT_ONE)
            await self.bot.reply(text.INIT_MULTIPLE.format(count))
        else:
            await self.bot.reply(text.INIT_NOBODY)

    @commands.command(pass_context=True, aliases=['bonbonalafraise'])
    async def reset(self, ctx: commands.Context):
        Person(ctx.message.author.id, self.redis).reset()
        await self.bot.reply(text.RESET)
    ...
    async def cleanse(self, ctx: commands.Context):
        if has_admin(self.bot.server, ctx.message.author):
            ...
            await self.bot.reply(text.CLEANSE)
        else:
    ...
    async def profile(self, ctx: commands.Context):
        ...
        if picture:
            ...
            await self.bot.reply(ctx.message.channel, text.PROFILE_FAILURE)
    ...
    async def thanks(self, ctx: commands.Context, member: str = None, reason: str = None):
        if member:
            ...
            if user.id == ctx.message.author.id:
                await self.bot.reply(text.THANKS_SELF)
            else:
            ...
            if thanks_count:
                ...
                await self.bot.reply(text.THANKS.format(thanks_count))
            else:
                await self.bot.reply(text.THANKS_NONE)
    ...
    async def available(self, ctx: commands.Context, cmd: str, *args):
        if cmd == 'oui' or cmd == 'non':
            ...
            if user:
                ...
                await self.bot.reply(text.AVAILABILITY_WRONG_ARGS)
    ...
    async def nick(self, ctx: commands.Context, cmd: str = None, input_title: str = None):
        if cmd:
            if cmd.startswith('<'):
                ...
                if not user_title:
                    await self.bot.reply(text.NO_NICK)
                else:
            ...
            if cmd == 'edit':
                if not input_title:
                    await self.bot.reply(text.USAGE_NICK)
                else:
                    if len(input_title) > 24:
                        await self.bot.reply(text.TOO_LONG_NICK)
                    else:
                        title = re.sub('[`\'"@&^%$!*()+=-]', '', input_title)
                        Person(ctx.message.author.id, self.redis).update('title', title)
                        await self.bot.reply(text.UPDATED_NICK.format(title))
        else:
            author = ctx.message.author
            user_title = Person(author.id, self.redis).title
            if not user_title:
                await self.bot.reply(text.NO_SELF_NICK)
            else:
    ...
    async def desc(self, ctx: commands.Context, cmd: str = None, input_desc: str = None):
        if cmd:
            if cmd.startswith('<'):
                ...
                if not user_desc:
                    await self.bot.reply(text.NO_DESC)
                else:
            ...
            if cmd == 'edit':
                if not input_desc:
                    await self.bot.reply(text.USAGE_DESC)
                else:
                    if len(input_desc) > 150:
                        await self.bot.reply(text.TOO_LONG_DESC)
                    else:
                        Person(ctx.message.author.id, self.redis).update('desc', input_desc)
                        await self.bot.reply(text.UPDATED_DESC.format(input_desc))
        else:
            author = ctx.message.author
            user_desc = Person(author.id, self.redis).desc
            if not user_desc:
                await self.bot.reply(text.NO_SELF_DESC)
            else:
                await self.bot.reply(text.HAS_SELF_DESC.format(user_desc.decode('utf-8')))
